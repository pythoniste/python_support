...
class RandomUDPHandler(socketserver.DatagramRequestHandler):
    def handle(self):
        capital, taux_annuel, durée_en_mois = (float(e) for e in self.rfile.readline().strip().decode("utf-8").split("|"))
        resultat = calcul_mensualité(capital, taux_annuel, durée_en_mois)
